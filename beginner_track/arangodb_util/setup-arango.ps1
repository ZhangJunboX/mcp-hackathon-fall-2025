...
for ($i = 0; $i -lt $maxTries; $i++) {
    ...
    if ($status -match "(healthy|Up)") {
        Write-Host "✓ ArangoDB container is $status" -ForegroundColor Green
        break
    }
    if ($i -lt $maxTries - 1) {
        Write-Host "Waiting for ArangoDB container... ($($i + 1)/$maxTries)"
...
Write-Host "Copying setup script to container..."
...
Write-Host "Done. Database '$DbName' and user '$User' ready." -ForegroundColor Green
Write-Host ""
Write-Host "Connection Details:"
Write-Host "  URL: http://localhost:8529"
Write-Host "  Database: $DbName"
Write-Host "  Username: $User"
Write-Host "  Web UI: http://localhost:8529 (login with root:$RootPassword)"
